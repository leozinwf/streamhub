    ...
    .map((line) => {
      ...
      if (trimmed.startsWith('#')) {
        return line.replace(/URI="([^"]+)"/gi, (_match, value: string) => {
          try { return `URI="${proxiedUrl(new URL(value, baseUrl), requestUrl)}"` } catch { return `URI="${value}"` }
        })
      }
      try { return proxiedUrl(new URL(trimmed, baseUrl), requestUrl) } catch { return line }
    })
    .join('\n')
}
...
export async function handleStream(req: Request): Promise<Response> {
  ...
  try {
    ...
    upstreamHeaders.Referer = requestReferer || `${target.origin}/`
    upstreamHeaders.Origin = requestOrigin || target.origin

    const upstream = await fetch(target, { signal: controller.signal, headers: upstreamHeaders, redirect: 'follow' })
    if (!upstream.ok) return new Response(`Servidor de origem respondeu HTTP ${upstream.status}.`, { status: upstream.status, headers: { 'Access-Control-Allow-Origin': '*' } })
    ...
    if (isManifest) {
      ...
      if (!manifestLike && looksLikeManifestUrl) return new Response('A origem respondeu algo que não é um manifesto HLS válido.', { status: 502, headers: { 'Content-Type': 'text/plain; charset=utf-8', 'Access-Control-Allow-Origin': '*' } })
      const rewritten = manifestLike ? rewriteManifest(content, req.url, finalUrl) : content
      return new Response(rewritten, { status: 200, headers: { 'Content-Type': 'application/vnd.apple.mpegurl; charset=utf-8', 'Cache-Control': 'no-store, no-cache, must-revalidate', Pragma: 'no-cache', 'Access-Control-Allow-Origin': '*' } })
    }

    const headers = responseHeaders(upstream.headers)
    if (!headers.has('content-type')) headers.set('Content-Type', /\.ts(?:$|\?)/i.test(finalUrl.pathname + finalUrl.search) ? 'video/mp2t' : 'application/octet-stream')
    return new Response(upstream.body, { status: upstream.status, headers })
  } catch (error) {
    const aborted = error instanceof DOMException && error.name === 'AbortError'
    return new Response(aborted ? 'Tempo limite ao acessar o stream.' : 'Não foi possível acessar o stream de origem.', { status: 502, headers: { 'Access-Control-Allow-Origin': '*' } })
  } finally {
